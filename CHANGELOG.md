...
- **Operational receptionist tools:** inbound agents can now persist tenant-scoped customers and idempotent leads, run deterministic safety and service-area gates, retrieve short-lived conflict-checked appointment slots, transactionally create jobs and appointments, send auditable Twilio SMS confirmations, manage existing appointments, create callback/escalation records, and save transcript-backed call summaries and technician briefs. Call-scoped workflow state remains in `SessionStore`, server-side organization scoping prevents model-supplied cross-tenant access, and post-call persistence supplies a deterministic fallback when callers hang up before explicit finalization.
...
- **OpenAI Realtime fragmented function arguments:** tool dispatch now waits for and reconciles `response.function_call_arguments.done` when `response.output_item.done` contains incomplete JSON. Both documented event orders are supported, preventing empty-parameter execution of side-effectful operational tools.
